...
import { blockSchema, createRunSchema, loginSchema, nextActionSchema, progressSchema, registerSchema, updateRunSchema } from './http/schemas'
...
export function createApp(storeFactory: StoreFactory = (env) => new D1RunStore(env.DB)) {
  ...
  app.get('/api/runs', async (c) => c.json({ data: await runs(c).list(c.get('user').id) }))
  ...
  app.get('/api/today', async (c) => c.json({ data: await runs(c).today(c.get('user').id) }))
